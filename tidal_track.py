...
def main():
    model_names = ["r=0.005", "r=0.008", "r=0.015", "r=0.025", "r=0.05"]
    #model_names = ["fid_dwarf", "r=0.005", "r=0.008", "r=0.015",
    #               "r=0.025", "r=0.05"]
    for name in model_names:
        plot_model(name, name == model_names[0], True)
        ...
        for i in range(1,len(gal)):
            ...
            if include_relax:
                ...
                for s in range(snap, len(scale)):
                    mrelax[s] = np.sum(stars[i]["mp"][ok][t_relax < dt[s]])
                    mrelax2[s] = np.sum(stars[i]["mp"][ok][t_relax/3 < dt[s]])
                    mrelax3[s] = np.sum(stars[i]["mp"][ok][t_relax/10 < dt[s]])
