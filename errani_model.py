...
# going to use the function eddington_inversion to compute the normalized energy
...
#
# eps is 1 - E/E0, where E0 is the energy of the deepest part of the potential.
# So eps = 0 is the most-bound state and anythign with eps > 1 is unbound.
#
...
# mass values.
#
#    n_t = # whatever you want, this is the number of points in the tidal track
...
# in he code snippet above, I didn't need to compute a full table of mf values,
# I should have just used a general optimization routine. I didn't even bother
# interpolating the final tidal track in the example.
#
...
# "NFW". You could also imagine writing a new profile type where the central
# galaxy has self gravity by adding the potenial generated by the stars to the
# profile.
profile_type = "Einasto"

# Set up cosmology. If you're not using symilb or colossus, you can change this.
...
# Plotting stuff. This is my plotting library. Feel free to get rid of it.
...
# if you're not using symlib, m_star_model needs to be an object with a method
# m_star_model.m_star(mvir, z) (solar masses without h's). r_half_model needs
# a method r_half_model.r_half(rvir, cvir, z).
...
elif profile_tpye == "Einasto":
    alpha = 0.16 # This is the correct alpha and I will fight you
...
# Everything below this point is random scripts that use the above code.
